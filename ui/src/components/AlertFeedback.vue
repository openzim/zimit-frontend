...
export default {
  ...
  beforeMount() {
    ...
    this.$root.$on("feedback-message", function (level, message, duration) {
      ...
      if (duration === undefined) duration = Constants.ALERT_DEFAULT_DURATION;
      if (duration === true) duration = Constants.ALERT_PERMANENT_DURATION;
