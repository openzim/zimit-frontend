<template>
  <div class="container">
    <div v-if="!error && task">
      <h1 v-if="task && task.config">
        {{ $t("request.zimingOf")
        }}<a :href="task.config.flags.url" target="_blank">{{
      ...
      <div v-if="failed">
        <b-alert fade show variant="danger">
          ...
          <p>
            {{ $t("request.failureReasons") }}
            <a href="https://github.com/openzim/zimit/issues/new">{{
              $t("request.openTicket")
            }}</a
            >.
          </p>
        </b-alert>
      ...
      <div v-if="succeeded">
        <b-alert fade show variant="success">
          ...
          <p>
            <a
              :href="
                zim_download_url + task.config.warehouse_path + '/' + file.name
              "
              ><b-button pill variant="grey">{{
                $t("request.download")
              }}</b-button></a
            >
          </p>
...
export default {
  ...
  props: {
    taskId: {
      type: String, // the zimfarm task ID
      required: true,
    },
  },
  data() {
  ...
  computed: {
    is_requested() {
      return this.task.status == "requested";
    },
    ended() {
      return this.succeeded === true || this.failed === true;
    },
    ongoing() {
      return this.succeeded !== true && this.failed !== true;
    },
    ...
    progression() {
      ...
      return this.task.container &&
        this.task.container.progress &&
        this.task.container.progress.overall
        ? this.task.container.progress.overall
        : 0;
    },
    visibility_fix() {
      if (this.progression < 15) return "visible-text";
      return "";
    },
    progress_variant() {
      if (this.succeeded === true) return "success";
      if (this.failed === true) return "danger";
      return "info";
    },
    simple_status() {
      if (this.is_requested) return this.$t("request.pending");
      if (this.ended) return this.task.status;
      return this.$t("request.inProgress");
    },
    sorted_files() {
      return Object.values(this.task.files).sortBy("created_timestamp");
    },
    file() {
      return Object.values(this.sorted_files)[0] || {};
    },
    zim_download_url() {
      return Constants.zim_download_url;
    },
    flags() {
      if (!this.task || !this.task.config || !this.task.config.flags) return {};
      return Object.entries(this.task.config.flags)
        .filter(
          ([key, val]) =>
            !Constants.hidden_flags.includes(key) &&
    ...
    limit_hit() {
      return (
        ...
        this.task.container.progress.limit.hit
      );
    },
  },
  mounted() {
    ...
    this.interval = setInterval(
      this.loadTask,
      Constants.zimit_refresh_after * 1000,
      false,
    );
  },
  methods: {
      ...
      this.queryAPI("get", `${Constants.zimitui_api}/requests/${this.task_id}`)
        ...
        .catch((error) => {
          ...
          if (error.response && error.response.status === 404) {
            ...
            this.alertError(
              this.$t("request.taskRetrieveError", {
