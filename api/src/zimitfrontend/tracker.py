...
from zimitfrontend.constants import logger, ApiConfiguration
...
class Tracker:
    ...
    def _ongoing_task_has_finished(self, task_id: str) -> bool:
        ...
        if not success:
            ...
            raise HTTPException(
                ...
                detail=f"Unable to find ongoing task {task_id} status via HTTP "
                f"{status}: {task}",
            )
    ...
    def _generate_unique_id(self) -> str:
        ...
        digest = hmac.new(ApiConfiguration.digest_key, identifier.encode(), "sha256").hexdigest()
    ...
    def _is_valid_unique_id(self, unique_id: str) -> bool:
        ...
        identifier = splits[0]
        digest = splits[1]
        expected_digest = hmac.new(ApiConfiguration.digest_key, identifier.encode(), "sha256").hexdigest()
    ...
    def add_task(
        ...
        # when we've reached this point, there is no ongoing task pending for this client
        ...
        if not unique_id:
            ...
            self.known_clients.append(ClientInfo(ip_address=ip_address, unique_id=new_unique_id, ongoing_tasks=[task_id]))
            return AddTaskResponse(
        ...
        if clients_info_by_unique_id := [
            ...
            self.known_clients.append(ClientInfo(ip_address=ip_address, unique_id=unique_id, ongoing_tasks=[task_id]))
