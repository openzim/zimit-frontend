...
from zimitfrontend.routes.schemas import TaskCancelRequest, TaskCreateRequest, TaskCreateResponse, TaskInfo
...
def create_task(request: TaskCreateRequest, http_request: Request, http_response: Response) -> TaskCreateResponse:
    ...
    if add_task.status != AddTaskStatus.CAN_ADD_TASK:
        raise HTTPException(
            ...
            detail={"message": "Too many requests already ongoing for your user", "reason": add_task.status.value, "ongoing_tasks": add_task.ongoing_tasks}
        )
...
def cancel_task(
    ...
    if task_id not in status.ongoing_tasks:
        raise HTTPException(
    ...
    if status == HTTPStatus.OK:
        ...
        if status != HTTPStatus.OK:
            raise HTTPException(
                status_code=HTTPStatus.BAD_REQUEST,
                detail={
                    "error": f"Failed to delete requested task on Zimfarm with HTTP {status}",
                    "zimfarm_message": task,
                },
            )
        return
        ...
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST,
            detail={
                "error": f"Failed to search for requested task on Zimfarm with HTTP {status}",
                "zimfarm_message": task,
            },
        )
    ...
    if status == HTTPStatus.OK:
        ...
        _, status, task = query_api("DELETE", f"/tasks/{task_id}")
        if status != HTTPStatus.OK:
            raise HTTPException(
                status_code=HTTPStatus.BAD_REQUEST,
                detail={
