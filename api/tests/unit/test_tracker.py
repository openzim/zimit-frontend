from zimitfrontend.tracker import AddTaskStatus, Tracker, ClientInfo
import pytest

from zimitfrontend.constants import ApiConfiguration

CLIENT_1_IP = "172.16.1.1"
CLIENT_1_ID = "cf068243341e4e979c35fd4fb82cea5d|7abc6ca76820e0d693d8a22ef0bf002f2f25e5c9f937a84998572c67e097e301"

CLIENT_2_IP = "172.16.1.2"
CLIENT_2_INVALID_ID = "id1"  # this is not a valid
CLIENT_2_VALID_ID = "c047287b89854bf9b64aa7a1caf359f8|8b6dbcad94abf2e872d5a066575241127eee3636bd9d7f25f8b68bb22e34c4d1"  # this is a valid

CLIENT_3_IP = "172.16.1.3"
CLIENT_3_ID = "751e636176714d45add32deea6f8931c|90ced10358a341684e7fb2fb46ce52702e9ac32ed497b20c05c9db913e025ad0"

CLIENT_4_IP = "172.16.1.4"
CLIENT_4_ID1 = "989536ffae664d0d9df62701424bedfc|cf6134ae38f74ab595880c1e9c1442df2a24edc2e24c045cde884c8ed72a0ea8"
CLIENT_4_ID2 = "d7c3ff46bc314ecfbc7268b452f24a32|d0a21077e88fd58ccaa40a489b9221f2af4468273525e7736677f5f903ab0cef"

CLIENT_5_IP1 = "172.16.1.5"
CLIENT_5_IP2 = "172.16.1.6"
CLIENT_5_ID = "634ecae47311413c9fd90725e08593e1|c31f98c5bd633785b77b2706a6eee35037bcc1356c75cddbe6478e6afe0f8023"
...
def tracker() -> Tracker:
    ...
    tracker.ongoing_task_has_finished = lambda task_id: task_id in [TASK_ID1, TASK_ID4]
    return tracker


@pytest.mark.parametrize(
    "ip_address, unique_id, task_id, expected_status, expected_ongoing_tasks, new_unique_id_is_set",
    [
        pytest.param(
            CLIENT_1_IP,
            CLIENT_1_ID,
...
def test_custom_max_tasks(tracker: Tracker):
    ...
    assert result.new_unique_id == None
    tracker.has_reached_maximum_tasks = lambda client: len(client.ongoing_tasks) > 1
    result = tracker.add_task(CLIENT_3_IP, CLIENT_3_ID, TASK_ID8)
    assert result.status == AddTaskStatus.TASK_ADDED
    assert result.ongoing_tasks == None
    assert result.new_unique_id == None
